...
/// ui.add(VideoSlider::new(&mut my_f32, 0.0..=100.0).text("My value"));
/// # });
/// ```
///
/// The default [`VideoSlider`] size is set by [`crate::style::Spacing::slider_width`].
...
impl<'a> VideoSlider<'a> {
    ...
    /// ui.add(VideoSlider::new(&mut my_i64, 0..=100).custom_formatter(|n, _| format!("{:X}", n as i64)));
    /// # });
    /// ```
    ...
    /// If you use one of the integer constructors (e.g. `VideoSlider::i32`) this is called for you,
    /// but if you want to have a slider for picking integer values in an `VideoSlider::f64`, use this.
...
impl<'a> VideoSlider<'a> {
    ...
    fn slider_rect(&self, position_1d: f32, rail_rect: &Rect) -> Rect {
        let slider_pos = self.marker_center(position_1d, rail_rect);
        let mut rect = rail_rect.clone();
        match self.orientation {
            SliderOrientation::Horizontal => rect.set_right(slider_pos.x),
            SliderOrientation::Vertical => rect.set_bottom(slider_pos.y),
        }
        rect
    }
...
impl<'a> Widget for VideoSlider<'a> {
